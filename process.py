...
Data = pd.read_csv(r'wikihowAll.csv', engine='python', encoding = "latin1")
...
for row in range(rows):
    ...
    if len(abstract) < (0.75*len(article)):
        ...
        data = filename + '.txt'
        filename1 = (data[:255] + '..') if len(data) > 255 else data
        ...
        with open(path+'/'+filename1,'wb') as f:
            ...
            with open('temporaryFile.txt','r',encoding = "latin1") as t:
                for line in t:
                    line=line.lower()
                    if line != "\n" and line != "\t" and line != " ":
                        f.write(b'@summary'+b'\n')
